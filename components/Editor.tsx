...
const Editor: React.FC<EditorProps> = ({ initialPresentation, onBack }) => {
  ...
  const handleGenerate = async (prompt: string) => {
    ...
    try {
      const newCode = await generateHtml(prompt, presentation.code.html, presentation.code.css, presentation.code.js, selectedElement?.selector || null);
      setPresentation(current => ({ ...current, code: newCode }));
  ...
  const handleCodeChange = (newHtml: string) => {
    ...
    if (elementToUpdate) {
        ...
        if (newElement) {
             ...
             setPresentation(current => ({...current, code: { ...current.code, html: updatedFullHtml }}));
    ...
    const handleIframeLoad = () => {
        ...
        const handleElementDblClick = (e: MouseEvent) => {
            ...
            handleBlur = () => {
                ...
                setPresentation(current => ({...current, code: { ...current.code, html: updatedHtml }}));
  ...
  const CodeView = () => (
    <div className="flex flex-col md:flex-row h-full w-full bg-gray-800 p-2 gap-2">
      <div className="flex flex-col w-full md:w-1/2 h-1/2 md:h-full">
        ...
        <textarea value={presentation.code.html} onChange={(e) => setPresentation(p => ({...p, code: {...p.code, html: e.target.value}}))} className="w-full h-full bg-gray-900 text-gray-200 p-2 border border-gray-700 rounded-md resize-none font-mono text-sm" />
      </div>
      <div className="flex flex-col w-full md:w-1/2 h-1/2 md:h-full gap-2">
        <div className="flex flex-col h-1/2">
            ...
            <textarea value={presentation.code.css} onChange={(e) => setPresentation(p => ({...p, code: {...p.code, css: e.target.value}}))} className="w-full h-full bg-gray-900 text-gray-200 p-2 border border-gray-700 rounded-md resize-none font-mono text-sm" />
        </div>
        <div className="flex flex-col h-1/2">
            <label className="text-sm font-bold text-gray-400 p-2">JavaScript</label>
            <textarea value={presentation.code.js} onChange={(e) => setPresentation(p => ({...p, code: {...p.code, js: e.target.value}}))} className="w-full h-full bg-gray-900 text-gray-200 p-2 border border-gray-700 rounded-md resize-none font-mono text-sm" />
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex flex-grow overflow-hidden">
        ...
        <div className="flex-grow flex items-center justify-center p-4 bg-gray-900 overflow-auto transition-all duration-300">
            {viewMode === 'code' ? <CodeView /> : (
                ...
                >
                    <iframe
                        ref={iframeRef}
                        srcDoc={getFullHtml(presentation.code)}
        ...
        {isVoiceOverPanelOpen && viewMode === 'editor' && (
            <div className="w-1/4 flex-shrink-0 p-4 bg-gray-800/50 border-l border-gray-700 overflow-y-auto transition-all duration-300">
                <VoiceOverPanel 
                    htmlContent={presentation.code.html}
                    originalPrompt={presentation.prompt} 
